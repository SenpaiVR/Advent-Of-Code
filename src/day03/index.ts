...
const part1 = (rawInput: string) => {
	...
	const sum = numbers.reduce((total, number) => {
		number[0] =
			typeof number[0] === "string" ? parseInt(number[0]) : number[0]
		number[1] =
			typeof number[1] === "string" ? parseInt(number[1]) : number[1]
		number[2] =
			typeof number[2] === "string" ? parseInt(number[2]) : number[2]
		...
		return isPartNumber ? total + number[0] : total
	}, 0)
